 ...
 * @since 1.0.0
 */
class Jet_Theme_Core_Plugin_Changelog {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		if ( $plugin === $plugin_file ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'jet-theme-core' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'jet-theme-core' )
			);
...
if ( ! function_exists( 'jet_theme_core_plugin_changelog' ) ) {
	function jet_theme_core_plugin_changelog() {
		return new Jet_Theme_Core_Plugin_Changelog();
	}
}
